...
export const PlayButton: React.FC<PlayButtonProps> = ({ handler }) => {
    ...
    const updateWindowDimensions = () => setVideoHeight(window.innerWidth/2.8);
    ...
    return (
        <>
            {
                modal ? <div className="modal w-[100%] h-[100%]" style={{ position: "fixed", top: "0", left: "0", right: "0", backgroundColor: "rgb(255,255,255,0.35)", zIndex: "1000" }} onClick={() => { setModal(!modal) }}>
                    <div className="modal-dialog w-[55%] h-[50%] m-auto mt-[7%]" style={{ zIndex: "1001" }}>
                        <div className="modal-content">
                            ...
                            <div className="modal-body">
                                <iframe
                                    width="100%"
                                    height={videoHeight}
                                    src="https://www.youtube.com/embed/z2OXnXu3_nc?controls=1&autoplay=1"
                                    title="Video Player"
            ...
            <div className="grid justify-items-center mt-[-125px] md:mt-[-170px] lg:mt-[-140px] gap-1 lg:gap-0 text-[14px] md:text-[20px] lg:text-[16px] font-[700]">
                <button onClick={() => setModal(!modal)} className="cursor-pointer grid w-[60px] h-[60px] md:w-[80px] md:h-[80px] lg:w-[72px] lg:h-[72px] hover:scale-[1.03] active:scale-100 pl-2 rounded-full bg-[#F97919] hover:bg-[#FFAF10] active:bg-[#F97919] transition-all duration-300 ease-out place-content-center">
